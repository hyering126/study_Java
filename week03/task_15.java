...
public class task_15 {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < mathScores.length; i++) {
            totalStudtent += mathScores[i].length; // 반의 학생 수 합산
            for (int k = 0; k < mathScores[i].length; k++) { // 해당 반의 학생 수 만큼 반복
                totalMathSum += mathScores[i][k]; // 학생 점수 합산
            }
        }
        double totalMAthAvg = (double) totalMathSum / totalStudtent;
        ...
        for (int i = 0; i < englishScores.length; i++) { // 반의 수만큼 반복
            totalStudtent += englishScores[i].length; // 반의 학생 수 합산
            for (int k = 0; k < englishScores[i].length; k++) { // 해당 반의 학생 수만큼 반복
                totalEnglishSum += englishScores[i][k]; // 학생 점수 합산
            }
        }
        double totalEnglishAvg = (double) totalEnglishSum / totalStudtent;
